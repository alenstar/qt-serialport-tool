#include "jvhelper.h"
JVHelper* JVHelper::__instance = 0;
JVHelper::JVHelper(QObject *) :
    QObject(qApp)
{
...
void JVHelper::SetIcon(QLabel *lab, QChar c, int size)
{
...
void JVHelper::SetIcon(QPushButton *btn, QChar c, int size)
{
    iconFont.setPointSize(size);
...
QIcon &JVHelper::GetAppIcon()
{
    return this->appIcon;
}

void JVHelper::SetAppIcon(QString name)
{
    this->appIcon.addFile(name, QSize(), QIcon::Normal, QIcon::Off);
}

bool JVHelper::TcpPortIsFreed(int port)
{
#if 0
...
QString JVHelper::FromBase64ForUrl(QString base64String)
{
...
/// </summary>
QString JVHelper::ToBase64StringForUrl(QString normalString)
{
...
JButton::JButton(QWidget *parent) :
    QPushButton(parent)
{
...
JLoader::JLoader(QString img, QWidget *parent) : QLabel(parent)
{
...
JLoader::JLoader(QPixmap& img, QWidget *parent) : QLabel(parent)
{
	this->timer = new QTimer();
	this->img = QPixmap(img);
	this->resize(this->img.size());
...
void CycloProgress::paintEvent(QPaintEvent * /*event*/)
{
	...
	if(! m_bg.isNull())
		painter.drawPixmap((m_pixmap.width() - m_bg.width()) / qreal(2), (m_pixmap.height() - m_bg.height()) / qreal(2), m_bg);
